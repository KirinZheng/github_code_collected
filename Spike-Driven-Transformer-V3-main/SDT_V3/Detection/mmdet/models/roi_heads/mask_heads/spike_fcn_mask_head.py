...
class MultiSpike_norm4(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            ...
            return torch.clamp(x, min=0, max=4).round_() / self.T


@MODELS.register_module()
class SpikeFCNMaskHead(BaseModule):
    ...
    def __init__(self,
        ...
        if self.upsample_method is None:
            ...
            upsample = build_upsample_layer(upsample_cfg_)
            self.upsample = nn.Sequential(
                # TODO(hujiakui): this lif may be removed
                MultiStepLIFNode(tau=2.0, detach_reset=True, backend="cupy"),
                # MultiSpike_norm4(T=T),
                SeqToANNContainer(upsample),
            )
        ...
        conv_logits = build_conv_layer(self.predictor_cfg,
                                       logits_in_channel, out_channels, 1)
        self.conv_logits = nn.Sequential(
            MultiStepLIFNode(tau=2.0, detach_reset=True, backend="cupy"),
            # MultiSpike_norm4(T=T),
            SeqToANNContainer(conv_logits),
        )
        # TODO(hujiakui): remove relu here
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # if x.dim == 4:
        #     x = x.unsqueeze(0)
        for conv in self.convs:  #这里可以改进bug，到第二层之后后面发放率都为0.但不是代码的问题。如果用残差应该会更好些
            x = conv(x)
        if self.upsample is not None:
            x = self.upsample(x)
            if self.upsample_method == 'deconv':
                x = self.relu(x)
        mask_preds = self.conv_logits(x).mean(dim=0)
        return mask_preds
