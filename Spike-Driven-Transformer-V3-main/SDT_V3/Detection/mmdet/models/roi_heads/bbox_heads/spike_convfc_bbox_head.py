...
class SpikeConvFCBBoxHead(BBoxHead):
    ...
    def __init__(self,
        ...
        self.cls_convs, self.cls_fcs, self.cls_last_dim = \
            self._add_conv_fc_branch(
                self.num_cls_convs, self.num_cls_fcs, self.shared_out_channels, T=T)

        # add reg specific branch
        self.reg_convs, self.reg_fcs, self.reg_last_dim = \
            self._add_conv_fc_branch(
                self.num_reg_convs, self.num_reg_fcs, self.shared_out_channels, T=T)
...
@MODELS.register_module()
class SharedSpike2FCBBoxHead(SpikeConvFCBBoxHead):
...
@MODELS.register_module()
class SharedSpike4Conv1FCBBoxHead(SpikeConvFCBBoxHead):
