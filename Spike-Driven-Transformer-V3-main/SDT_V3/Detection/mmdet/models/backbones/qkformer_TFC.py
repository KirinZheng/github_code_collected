...
class Spiking_Self_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = k.transpose(-2,-1) @ v  # k: T B head C N, v: T B head N C , result T B head C C
        x = (q @ x) * self.scale    # T B head N C
...
class hierarchical_spiking_transformer_TFC(BaseModule):
    def __init__(self,
                 T=4,
                 img_size_h=128, img_size_w=128, patch_size=16, in_channels=2, num_classes=11,
                 embed_dims=[64, 128, 256], num_heads=[1, 2, 4], mlp_ratios=[4, 4, 4], qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], recurrent_coding=False, init_cfg=None,
                 ):
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            assert 'checkpoint' in self.init_cfg, (
                f'Only support specify `Pretrained` in `init_cfg` for {self.__class__.__name__}'
            )
            ...
            if 'model' in ckpt:
                ...
                for k, v in before_state_dict.items():
                    _state_dict[k] = v  # 不用删除 "backbone."，因为ckp 的 state_dict 并没有 对应前缀

            # 获取模型当前的参数 key
            model_keys = set(self.state_dict().keys())
            ckpt_keys = set(_state_dict.keys())
            ...
            # ✅ 日志输出
            logger.info(f"[INIT] ✅ Loaded params from checkpoint: {len(matched_keys)}")
            if missing_keys:
                logger.warning(f"[INIT] ⚠️ Missing params not in checkpoint ({len(missing_keys)}):")
                for k in sorted(missing_keys):
                    logger.warning(f"    - {k}")
            if unexpected_keys:
                logger.warning(f"[INIT] ⚠️ Unused params in checkpoint ({len(unexpected_keys)}):")
            ...
            # 也可选输出 PyTorch 的 load 结果（详细）
            logger.info(str(load_result))
...
def QKFormer_10_384(T=1, **kwargs):
    model = hierarchical_spiking_transformer_TFC(
        T=T,
        img_size_h=224, img_size_w=224,
        patch_size=16, embed_dims=384, num_heads=6, mlp_ratios=4,
        in_channels=3, num_classes=1000, qkv_bias=False,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=10, sr_ratios=1, recurrent_coding=True, 
        **kwargs
    )
    return model
