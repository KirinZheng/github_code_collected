...
class SepConv(nn.Module):
    ...
    def __init__(
        ...
        if first_layer:
            self.encode_conv_tfc = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1)
            self.encode_bn_tfc = nn.BatchNorm2d(dim)

    def forward(self, x):
        T, B, C, H, W = x.shape
        if hasattr(self, "encode_conv_tfc"):
            t_x = []
            for i in range(T):
                if i == 0:
                    x_in = x[i]
                ...
                x_out = self.lif1(x_in.unsqueeze(0)).squeeze(0)  # B C H W
                tmp = x_out
                x_out = self.encode_conv_tfc(x_out)
                x_out = self.encode_bn_tfc(x_out)       # B C H W
...
class MS_ConvBlock(nn.Module):
    def __init__(
        ...
        self.Conv = SepConv(dim=dim, first_layer=first_layer)
...
class MS_DownSampling(nn.Module):
    def __init__(
        ...
        # design by 3d_pe_arch_4
        if not first_layer:
            # print(first_layer, in_channels)
            self.encode_lif = MultiStepLIFNode(
                tau=2.0, detach_reset=True, backend="cupy")
            
            # self.encode_conv_tfc = nn.Conv2d(
            #                     in_channels,
            #                     in_channels,
            #                     kernel_size=3,
            #                     stride=2,
            #                     padding=1,
            #                 )
            # self.encode_bn_tfc = nn.BatchNorm2d(in_channels)

            # pos_embed = get_3d_sincos_pos_embed(embed_dim=in_channels, spatial_size=(),
            #                                     temporal_size=1, output_type="pt",
            #                                     )  # T HW D
    ...
    def forward(self, x):
        ...
        if hasattr(self, "encode_lif"):
            ...
            #     x_out = self.encode_lif(x_in.unsqueeze(0)).squeeze(0)      # B C H W
            #     tmp = x_out

            #     x_out = self.encode_conv_tfc(x_out)
            #     print("here1")
            #     print(x_out.shape)
            #     x_out = self.encode_bn_tfc(x_out)
            #     print("here2")
            #     print(x_out.shape)
...
class Spiking_vit_MetaFormer_TFC(BaseModule):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
        ...
        self.ConvBlock1_1 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[0] // 2, mlp_ratio=mlp_ratios, first_layer=True)]
        )
...
def spikformer_8_384_CAFormer(**kwargs):
    model = Spiking_vit_MetaFormer_TFC(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
