...
pretrained = "/zhengzeqi/top_down/spikedrivenv2/weight/55M_kd.pth"  # 55M_coco_kd imagenet
...
model = dict(
    ...
    backbone=dict(
        _delete_=True,
        type='Spiking_vit_MetaFormer_TFC',
        ...
        depths=8,
        sr_ratios=1,
        T=2,            # 注意这里的T=1，会报错，因为x_out 变量用不到DDP error
    ...
    neck=dict(
        ...
        out_channels=512,     # 55M
        # out_channels=256,   # 15M
        num_outs=5),
    rpn_head=dict(
        type='SpikeRPNHead',
        # in_channels=256,   # 15M
        # feat_channels=256, # 15M
        in_channels=512,   # 55M
        feat_channels=512, # 55M
    ...
    roi_head=dict(
        ...
        mask_head=dict(
            ...
            num_convs=4,
            # in_channels=256,         # 15M
            # conv_out_channels=256,   # 15M
            in_channels=512,         # 55M
            conv_out_channels=512,   # 55M
            num_classes=80,
            loss_mask=dict(
