...
# import positional_encoders, caches
...
class DenseFormer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            # 这里按照n_repeat划分组，每个组里面又是increase_T_every来创建blocks
            # assert config.n_layer % self.increase_T_every == 0 保证了整数划分
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_format', default='base', choices=config_tmp.registered_formats())

    args, rem_args = parser.parse_known_args()

    config = config_tmp.parse_args_with_format(format=args.config_format, base_parser=parser, args=rem_args, namespace=args)
